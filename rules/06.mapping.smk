...
rule STAR_mapping:
    ...
    output:
        ...
        UniqueMultiple_str1 = "02.mapping/STAR/{sample}/{sample}.Signal.UniqueMultiple.str1.out.bg"
        UniqueMultiple_str2 = "02.mapping/STAR/{sample}/{sample}.Signal.UniqueMultiple.str2.out.bg"
        Unique_str1 =  "02.mapping/STAR/{sample}/{sample}.Signal.Unique.str1.out.bg"
        Unique_str2 =  "02.mapping/STAR/{sample}/{sample}.Signal.Unique.str2.out.bg"
    resources:
...
rule compress_bg:
"""
    Module: Post-processing / Compression
    Function: Compress BedGraph (.bg) files generated by STAR using pigz.
    
    Why: 
    1. STAR's raw signal output (.bg) is uncompressed and can consume significant disk space.
    2. pigz (Parallel Gzip) is used to speed up the process by utilizing multiple CPU cores.
    """
    input:
        UniqueMultiple_str1 = "02.mapping/STAR/{sample}/{sample}.Signal.UniqueMultiple.str1.out.bg",
        UniqueMultiple_str2 = "02.mapping/STAR/{sample}/{sample}.Signal.UniqueMultiple.str2.out.bg",
        ...
        Unique_str2 =  "02.mapping/STAR/{sample}/{sample}.Signal.Unique.str2.out.bg",
    output:
        UniqueMultiple_str1_gz = "02.mapping/STAR/{sample}/{sample}.Signal.UniqueMultiple.str1.out.bg.gz",
        UniqueMultiple_str2_gz = "02.mapping/STAR/{sample}/{sample}.Signal.UniqueMultiple.str2.out.bg.gz",
        Unique_str1_gz =  "02.mapping/STAR/{sample}/{sample}.Signal.Unique.str1.out.bg.gz",
        Unique_str2_gz =  "02.mapping/STAR/{sample}/{sample}.Signal.Unique.str2.out.bg.gz",
    resources:
        **rule_resource(config, 'medium_resource_resource',  skip_queue_on_local=True,logger = logger),
    conda:
        workflow.source_path("../envs/pigz.yaml"),
    message:
        "Running compress {wildcards.sample} star bg file",
    log:
        "logs/02.mapping/bg_compress_{sample}.log",
    benchmark:
            "benchmarks/{sample}_bg_compress_benchmark.txt",
    threads:
        config['parameter']['threads']['compress_bg'],
    shell:
        """
        ( pigz -11 -v -p {threads} {input.UniqueMultiple_str1}
        pigz -11 -v -p {threads} {input.UniqueMultiple_str2}
        pigz -11 -v -p {threads} {input.Unique_str1}
        pigz -11 -v -p {threads} {input.Unique_str2} ) &>{log}
        """
...
rule rseqc_tin:
    """
    Calculate Transcript Integrity Number (TIN) using RSeQC.

    TIN is a metric to measure RNA integrity at the transcript level. It is calculated
    by analyzing the uniformity of read coverage across the length of a transcript.
    - TIN = 100: Perfectly uniform coverage (high integrity)
    - TIN = 0: No coverage or highly degraded (low integrity)

    This metric is often more sensitive and informative than global RIN (RNA Integrity Number)
    scores for assessing sample quality in RNA-seq experiments.
    """
    input:
        bam = '02.mapping/STAR/sort_index/{sample}.sort.bam',
        bai = '02.mapping/STAR/sort_index/{sample}.sort.bam.bai',
    ...
    message:
        "Calculating TIN score for {wildcards.sample}"
    log:
        "logs/02.mapping/rseqc_tin_{sample}.log"
    benchmark:
        "benchmarks/{sample}_rseqc_tin.txt"
    threads: 1
    shell:
        """
...
rule geneBody_coverage:
    ...
    Input:  BigWig file (generated by bamCoverage).
    Output: Coverage statistics text, R plotting script, and PDF plot.
    """
    input:
        bw = f"02.mapping/bamCoverage/{{sample}}_{config['parameter']['bamCoverage']['normalizeUsing']}.bw"
    output:
        geneBody = "02.mapping/bamCoverage/{sample}.geneBodyCoverage.txt",
        r        = "02.mapping/bamCoverage/{sample}.geneBodyCoverage_plot.r",
        pdf      = "02.mapping/bamCoverage/{sample}.geneBodyCoverage.pdf",
    resources:
        **rule_resource(config, 'low_resource', skip_queue_on_local=True, logger=logger),
    conda:
        workflow.source_path("../envs/rseqc.yaml"),
    message:
        "Calculating gene body coverage (RSeQC) for {input.bw}"
    log:
        "logs/02.mapping/geneBodyCoverage_{sample}.log",
    benchmark:
        "benchmarks/{sample}_geneBodyCoverage_benchmark.txt",
    threads:
        1
    params:
        bed12 = config['STAR_index'][config['Genome_Version']]['bed12'],
        output_pri = "02.mapping/bamCoverage/{sample}"
    shell:
        """
        geneBody_coverage2.py -i {input.bw} \
                             -r {params.bed12} \
                             -o {params.output_pri} \
                             &> {log}
        """
