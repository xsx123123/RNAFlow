...
rule generate_reference_yaml:
    ...
    run:
        ...
        lines = [
            ...
            f"    {name}:",
            f"      index: {workflow_base}/{prefix}                                   # STAR INDEX CREATE BY RSEM",
            f"      genome_fa: {rel(input.genome_fa)}        # GENOME FASTA FILE",
            f"      genome_gtf: {rel(input.genome_gtf)}               # GENOME ANNOTATION FILE",
            f"      genome_gff: {rel(input.genome_gff)}              # GENOME ANNOTATION FILE",
            f"      rsem_index: {workflow_base}/{prefix}/{prefix}      # RSEM INDEX",
            f"      rsem_index_dir: {workflow_base}/{prefix}/                         # RSEM INDEX PATH",
            f"      bed12: {workflow_base}/{prefix}.bed12                  # GENE RANGES FOR GENE COVERAGE ANALYSIS",
            f"      go_annotation: {rel(input.go)}              # GO TERM ANNOTATION",
            f"      ref_all: {workflow_base}/{prefix}_ref_all.txt                           # gene information",
            "",
            "# 4. Append to 'deg_enrich_wrapper:' section",
            f"  {name}:",
            f"    gene_col: '{params.gene_col}'   # Column name for gene identifiers in deg result",
            "",
