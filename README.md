...
- **实时监控 (Real-time Monitoring)**：集成 Seq 日志服务器，通过定制化插件 `snakemake_logger_plugin_rich_loguru` 实现实时流程监控，支持结构化日志推送与可视化查询。
...
### 5. Seq 实时监控配置 (可选)
RNAFlow 支持通过 Seq 服务器进行实时流程监控，所有日志将被结构化推送至 Seq 服务器以供查询和分析：

- **`monitor_config.yaml`**: Seq 监控配置文件（可选，若不存在将使用 config.yaml 中的配置）
  ```yaml
  seq_server_url: "http://122.205.70.103:5341"  # Seq 服务器地址
  project_name: "MySnakemakePipeline"            # 项目名称（将自动添加时间戳后缀）
  # api_key: "your-api-key-here"                 # API 密钥（可选）
  ```

- **配置参数**：
  - `seq_server_url`: Seq 服务器的访问地址
  - `project_name`: 项目名称，系统会自动追加时间戳后缀（格式：`YYYY-MM-DD_HH-mm`）以便区分不同运行批次
  - `api_key`: 访问密钥（可选，用于受保护的 Seq 服务器）
...
- **查询日志**：在 Seq 服务器中可通过以下方式进行日志查询：
  ```
  Project = 'MySnakemakePipeline_2026-01-29_16-45'
  ```

### Seq 服务器部署 (Docker)
要使用 Seq 监控功能，您需要先部署 Seq 服务器。以下是使用 Docker 部署的示例：

1. **生成管理员密码哈希**：
   ```bash
   echo 'zj747812' | docker run --rm -i datalust/seq config hash
   ```

2. **创建 Docker Compose 文件 (compose.yml)**：
   ```yaml
   version: '3.8'
   ...
   services:
     seq:
       ...
       environment:
         - ACCEPT_EULA=Y
         # 下面这一行，把你刚才生成的长字符串粘贴替换掉 "把你的Hash粘贴在这里"
         - SEQ_FIRSTRUN_ADMINPASSWORDHASH=QHQq22ghQaFJZCrui9G5L6fABbvipviD6gNOoSF55IS9Vd8nSzH4ePwt+F0WRpOaqECLnvY/OM+VheyixocJQvH8JgmV31gjsScVfiftHLnx
       ports:
         # 宿主机端口:容器端口
         # 访问 Web 界面用 http://localhost:8080
         - "8080:80"
         # API 接收日志端口 (Loguru 推送用这个)
         - "5341:5341"
       volumes:
         # 数据持久化，对应当前目录下的 seq_data 文件夹
         - ./seq_data:/data
   ```
...
- **Feature**: 使用定制化插件`snakemake_logger_plugin_rich_loguru`结合`seq`实现对于分析流程的监控。通过使用定制化插件实现了分析流程的监控,如果想要实现这个操作,需要自行搭建seq监控,同时修改config文件中的seq_server_url: "http://122.205.70.103:5341" # seqserver api 配置,同时最终推送到seq的信息可以使用Project = 'MySnakemakePipeline'的形式进行查询,同时注意推送到seq的project_id汇总后面添加时间后缀,已进行区分。
...
---
**Author**: JZHANG | **Version**: RNAFlow_v0.1.7
